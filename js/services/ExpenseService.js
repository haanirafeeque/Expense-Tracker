// SERVICE: stores and manages all data shared between controllers
app.service('ExpenseService', function() {

  var expenses = [];
  var budgets  = {};

  // Load saved expenses from localStorage (or use sample data on first run)
  if (localStorage.getItem('expenses')) {
    expenses = JSON.parse(localStorage.getItem('expenses'));
    for (var i = 0; i < expenses.length; i++) {
      expenses[i].date = new Date(expenses[i].date); // dates are saved as strings, convert back
    }
  } else {
    var today = new Date();
  ...
  function save() { localStorage.setItem('expenses', JSON.stringify(expenses)); } // save to browser

  this.activeEditExpense = null; // stores expense being edited so the form can read it
  ...
  this.addExpense = function(expense) {
    expense.id     = Date.now();                 // unique ID from timestamp
    expense.amount = parseFloat(expense.amount); // ensure it's a number
  ...
  this.deleteExpense = function(id) {
    for (var i = 0; i < expenses.length; i++) {
      if (expenses[i].id === id) { expenses.splice(i, 1); break; } // splice removes 1 item
    }
    save();
  };

  this.getBudgets  = function() { return budgets; };
  this.saveBudgets = function(b) { budgets = b; localStorage.setItem('budgets', JSON.stringify(b)); };
